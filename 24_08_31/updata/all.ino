...
const long interval = 4000;  // 1초마다 RPM 계산
...
void loop() {
  if (Serial.available() > 0) {
    ...
    }else if(input.startsWith("M1_ALL"){
      int fanSpeed = input.substring(8).toInt();  // 입력된 RPM 값
      targetRPMs[0] = fanSpeed;
      targetRPMs[1] = fanSpeed;
      targetRPMs[2] = fanSpeed;
    }else if(input == "start") {
      startReceived = true;
    }
  }
