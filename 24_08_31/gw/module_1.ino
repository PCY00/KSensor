...
bool targetReached[fanCount] = {false, false, false}; // 목표 RPM 도달 여부 추적
bool print_ = false;
int realRPMs[fanCount] = {0, 0, 0};
...
bool m1AllCommandReceived = false; // M1_All 명령 수신 여부
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // 새로운 입력 형식 M1_All 처리
    if (input.startsWith("M1_All")) {
      int fanSpeed = input.substring(7).toInt();  // 입력된 RPM 값
      for (int i = 0; i < 3; i++) {
        targetRPMs[i] = fanSpeed;
        targetReached[i] = false; // 새로운 목표 RPM이 설정되면 도달 플래그 초기화
      }
      m1AllCommandReceived = true;  // M1_All 명령이 수신되었음을 기록
    } else if (input == "start") {
      startReceived = true;
    }
  }
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    // M1_All 명령이 수신되었고 모든 팬이 목표 속도에 도달했을 경우 출력
    if (m1AllCommandReceived && targetReached[0] && targetReached[1] && targetReached[2]) {
      Serial.print("M1_All_");
      Serial.print(realRPMs[0]);
      Serial.print("_");
      Serial.print(realRPMs[1]);
      Serial.print("_");
      Serial.println(realRPMs[2]);
      m1AllCommandReceived = false;  // 출력 후 플래그 초기화
    }
  }
}
