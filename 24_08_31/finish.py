...
url_m = "http://114.71.220.59:2021/Mobius/SMLplatform/Sensor"
apm2_headers = {
...
def send_sejong_data_two(data, url):
    ...
    module_1_arr = []
    for item in module_1:
        try:
            module_1_arr.append(float(item))
        except ValueError:
            module_1_arr.append(item)
            
    module_2_arr = []
    for item in module_2:
        try:
            module_2_arr.append(float(item))
        except ValueError:
            module_2_arr.append(item)
            
    module_3_arr = []
    for item in module_3:
        try:
            module_3_arr.append(float(item))
        except ValueError:
            module_3_arr.append(item)
    ...
    con0_2_sejong_json = OrderedDict([
        ("label", float(where_list[2])),
        ("data", module_2_arr)
    ])
    con0_2_sejong_str = json.dumps(con0_2_sejong_json)
    
    con0_3_sejong_json = OrderedDict([
        ("label", float(where_list[2])),
        ("data", module_3_arr)
    ])
    con0_3_sejong_str = json.dumps(con0_3_sejong_json)

    
    data_sejong1 = json.dumps({
        "m2m:cin": {
            "con": con0_1_sejong_str
        }
    })
    data_sejong2 = json.dumps({
        "m2m:cin": {
            "con": con0_2_sejong_str
        }
    })

    data_sejong3 = json.dumps({
        "m2m:cin": {
            "con": con0_3_sejong_str
        }
    })


    r_apm1 = requests.post(url + "/module_1", headers=sejong_headers, data=data_sejong1)
    r_apm2 = requests.post(url + "/module_2", headers=sejong_headers, data=data_sejong2)
    r_apm3 = requests.post(url + "/module_3", headers=sejong_headers, data=data_sejong3)
#r_apm2.raise_for_status()
#   except requests.exceptions.RequestException as req_err:
#        print("Sejong request error:", req_err)
#    except requests.exceptions.HTTPError as http_err:
                ...
                if "error" in con0_1:
                    ...
                    if time_s and pm_data and pm_data_a:
                        ...
                        if len(data_list) != len(original_order):
                            ...
                            send_sejong_data_two(combined_data,url_m)
                    else:
